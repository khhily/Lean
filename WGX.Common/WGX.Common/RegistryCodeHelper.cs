...
{
    public class RegistryCodeHelper
    {
        public static string GenRegistryCode(string name)
        {
            if (name == "") return "";
            var regCode = GetMd5(name);

            //取前六位, 后六位
            if (!string.IsNullOrEmpty(regCode))
            {
                var preStr = regCode.Substring(0, 6);
                var endStr = regCode.Substring(regCode.Length - 6);

                var midStr = regCode.Substring((regCode.Length - 6) / 2, (regCode.Length - 6) / 2 + 6);
                var finalReg = preStr + midStr + endStr;

                var finalRegCode = GetMd5(finalReg);

                preStr = regCode.Substring(0, 6);
                endStr = regCode.Substring(regCode.Length - 6);

                midStr = regCode.Substring((regCode.Length - 6) / 2, (regCode.Length - 6) / 2 + 6);

                return preStr + midStr + endStr;
            }
            return "";
        }

        public bool CheckRegistryCode(string name, TimeSpan ts)
        {
            return false;
        }
